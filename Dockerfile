...
RUN git clone --branch release-1.10.0 https://github.com/google/googletest \
    /root/gtest
RUN cmake -B /root/gtest-build -DCMAKE_INSTALL_PREFIX=/root/gtest-install \
    -G Ninja -S /root/gtest
...
RUN wget -qO- http://releases.llvm.org/9.0.0/${CLANG_VERSION}.tar.xz \
    | unxz | tar -x
